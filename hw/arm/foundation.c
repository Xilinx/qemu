...
#define FOUNDATION_BOARD_ID 0x8e0
#define FOUNDATION_FLASH_SIZE (64 * 1024 * 1024)
#define FOUNDATION_FLASH_SECT_SIZE (256 * 1024)
                      ...
                      hwaddr periphbase, qemu_irq *pic)
{
    ...
    for (n = 0; n < smp_cpus; n++) {
        ...
        object_property_set_int(cpuobj, 0x411fd000, "midr", &error_abort);
                                   ...
                                   qemu_irq *pic)
{
    ...
    memory_region_init_ram(ram2, NULL, "vexpress.highmem2", 32 * 1024 * 1024,
                           &error_abort);
    vmstate_register_ram_global(ram2);
    /* RAM is from 0x80000000 upwards; there is no low-memory alias for it. */
    memory_region_add_subregion(sysmem, 0x88000000, ram2);
...
static void vexpress_modify_dtb(const struct arm_boot_info *info, void *fdt)
{
    ...
    acells = qemu_fdt_getprop_cell(fdt, "/", "#address-cells", 0, false, NULL);
    scells = qemu_fdt_getprop_cell(fdt, "/", "#size-cells", 0, false, NULL);
                                          ...
                                          DriveInfo *di)
{
    ...
    qdev_prop_set_uint32(dev, "num-blocks",
                         FOUNDATION_FLASH_SIZE / FOUNDATION_FLASH_SECT_SIZE);
    qdev_prop_set_uint64(dev, "sector-length", FOUNDATION_FLASH_SECT_SIZE);
...
static void foundation_common_init(VEDBoardInfo *daughterboard,
                                   MachineState *machine)
{
    ...
    MemoryRegion *sram = g_new(MemoryRegion, 1);
    MemoryRegion *trom = g_new(MemoryRegion, 1);
    MemoryRegion *tsram = g_new(MemoryRegion, 1);
    MemoryRegion *tdram = g_new(MemoryRegion, 1);
    MemoryRegion *flashalias = g_new(MemoryRegion, 1);
    MemoryRegion *flash0mem;
    const hwaddr *map = daughterboard->motherboard_map;
    int i;

    daughterboard->init(daughterboard, machine->ram_size, machine->cpu_model, pic);
    ...
    if (map[VE_NORFLASHALIAS] != -1) {
        ...
        memory_region_init_alias(flashalias, NULL, "vexpress.flashalias",
                                 flash0mem, 0, FOUNDATION_FLASH_SIZE);
    ...
    memory_region_init_ram(trom, NULL, "vexpress.trom", 32 * 1024 * 1024,
                           &error_abort);
    vmstate_register_ram_global(trom);
    memory_region_add_subregion(sysmem, map[VE_TROM], trom);

    memory_region_init_ram(tsram, NULL, "vexpress.tsram", 256 * 1024,
                           &error_abort);
    vmstate_register_ram_global(tsram);
    memory_region_add_subregion(sysmem, map[VE_TSRAM], tsram);

    memory_region_init_ram(tdram, NULL, "vexpress.tdram", 32 * 1024 * 1024,
                           &error_abort);
    vmstate_register_ram_global(tdram);
    memory_region_add_subregion(sysmem, map[VE_TDRAM], tdram);
    ...
    daughterboard->bootinfo.board_id = FOUNDATION_BOARD_ID;
...
static void foundation_init(MachineState *machine)
{
    foundation_common_init(&a15_daughterboard, machine);
}

static QEMUMachine foundation_machine = {
    .name = "foundation",
    .desc = "ARM Foundation model",
    .init = foundation_init,
...
{
    qemu_register_machine(&foundation_machine);
}
